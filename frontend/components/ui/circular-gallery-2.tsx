...
export interface CircularGalleryApi {
  prev: () => void;
  next: () => void;
  /** Move by `delta` pages (negative = back). One page = `itemsPerPage` items. */
  goByPages: (delta: number) => void;
}
...
interface CircularGalleryProps
  ...
  /**
   * Exposes prev/next pagination controls to a parent.
   */
  apiRef?: React.MutableRefObject<CircularGalleryApi | null>;
  /**
   * How many items one prev/next click advances. @default 3
   */
  itemsPerPage?: number;
}
...
function createTextTexture(
  ...
  context.font = font;
  // Pull the pixel size out of a font string like "700 30px <family>"
  // (parseInt would grab the font-weight "700" instead of "30").
  const sizeMatch = font.match(/(\d+(?:\.\d+)?)px/);
  const fontSize = sizeMatch ? parseFloat(sizeMatch[1]) : 30;

  // Wrap long captions, but keep lines wide so labels stay short (1-2 lines).
  const maxLineWidth = fontSize * 24;
  const lines = wrapLines(context, text, maxLineWidth);
  const lineHeight = Math.ceil(fontSize * 1.3);
  const widest = Math.max(
    ...lines.map((l) => Math.ceil(context.measureText(l).width)),
  );

  const padX = Math.round(fontSize * 0.9);
  const padY = Math.round(fontSize * 0.6);
  canvas.width = widest + padX * 2;
  canvas.height = lineHeight * lines.length + padY * 2;

  // rounded "pill" background for legibility over photos
  const r = Math.min(canvas.height / 2, fontSize * 0.9);
  context.clearRect(0, 0, canvas.width, canvas.height);
  context.fillStyle = "rgba(8, 11, 11, 0.66)";
  context.beginPath();
  ...
  lines.forEach((l, i) => {
    context.fillText(l, canvas.width / 2, startY + i * lineHeight);
  });
...
class Title {
  ...
  createMesh() {
    const { texture, width, height, lines } = createTextTexture(
      this.gl,
      this.text,
      this.font,
      this.textColor,
    ...
    // grow the label height with the number of lines so each line stays legible
    let labelHeight =
      this.plane.scale.y * 0.07 * Math.min(Math.max(lines, 1), 5);
    // never let the label take more than ~40% of the card height
    labelHeight = Math.min(labelHeight, this.plane.scale.y * 0.4);
    let labelWidth = labelHeight * aspect;
    // keep the label within the image width
    const maxWidth = this.plane.scale.x * 0.9;
    if (labelWidth > maxWidth) {
      labelHeight *= maxWidth / labelWidth;
      labelWidth = maxWidth;
    }
    this.mesh.scale.set(labelWidth, labelHeight, 1);
    // sit the label inside the card, near the bottom edge
    this.mesh.position.y =
      -this.plane.scale.y * 0.5 + labelHeight * 0.5 + this.plane.scale.y * 0.04;
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // "contain" fit: show the whole image (e.g. a certificate) without
          // cropping; the leftover margin is transparent.
          float planeAspect = uPlaneSizes.x / uPlaneSizes.y;
          float imageAspect = uImageSizes.x > 0.0
          ...
          gl_FragColor = vec4(color.rgb, color.a * inside * boxAlpha);
        }
      `,
      uniforms: {
        tMap: { value: texture },
  ...
  onResize(
    ...
    if (viewport) {
      this.viewport = viewport;
      const uniforms = this.plane.program.uniforms as Record<
        string,
        { value: number[] }
      >;
...
class App {
  ...
  onWheel(e: WheelEvent) {
    const delta = e.deltaY || (e as WheelEvent & { wheelDelta?: number }).wheelDelta || e.detail;
  ...
  paginate(direction: number, count: number) {
    if (!this.medias || !this.medias[0]) return;
    const width = this.medias[0].width;
    this.scroll.target += direction * count * width;
    this.onCheck();
  }
...
const CircularGallery = ({
  ...
  useEffect(() => {
    ...
    if (apiRef) {
      apiRef.current = {
        prev: () => app.paginate(-1, itemsPerPage),
        next: () => app.paginate(1, itemsPerPage),
        goByPages: (delta: number) =>
          app.paginate(Math.sign(delta), Math.abs(delta) * itemsPerPage),
      };
    }
  ...
  }, [items, bend, borderRadius, scrollSpeed, scrollEase, fontClassName, apiRef, itemsPerPage]);
