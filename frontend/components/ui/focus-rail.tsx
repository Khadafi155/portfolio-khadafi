...
export type FocusRailItem = {
  ...
  description?: string;
  imageSrc: string;
  /** Optional backdrop behind the (often cut-out) main image. */
  bgSrc?: string;
  href?: string;
  meta?: string;
};
...
interface FocusRailProps {
  ...
  showInfo?: boolean;
  /** Show the prev/next controls + counter. Default true. */
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <AnimatePresence mode="popLayout">
          ...
          >
            <img
              src={activeItem.bgSrc ?? activeItem.imageSrc}
              alt=""
      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center px-4 md:px-8">
        ...
        <motion.div
          className="relative mx-auto flex h-full min-h-[640px] w-full max-w-6xl items-center justify-center cursor-grab active:cursor-grabbing"
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[320px] md:w-[460px] rounded-2xl border-t border-white/20 bg-neutral-900 shadow-2xl transition-shadow duration-300",
                ...
                {item.bgSrc ? (
                  <>
                    ...
                    <img
                      src={item.imageSrc}
                      alt={item.title}
                      className="absolute bottom-0 left-1/2 h-[96%] w-auto max-w-full -translate-x-1/2 object-contain pointer-events-none"
                    />
                  </>
                ) : (
                  <img
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          ...
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-32 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                {activeItem.meta && (
                  <span className="text-xs font-medium uppercase tracking-wider text-[#15c8b6]">
                    {activeItem.meta}
                  </span>
                )}
